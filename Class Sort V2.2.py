...
def normaliseNames(df, colName):
    ...
    Args:
        ...
        colName: Column of names

    Returns:
        df: Dataframe with names normalised
    """
    for index, row in df.iterrows():
        if ',' in str(row[colName]):
            ...
            newNameString = row[colName]
        df.loc[index, colName] = newNameString
    return df
...
def shuffle(df, scores, options, ranges, numClasses):
    ...
    for col in options.keys():
        if str(options[col]) not in ['-1', '1']:#col not in ['numClasses', 'Name', 'numAttempts', 'Firstname', 'Surname', 'Class']:
            for i in range (int(ranges[col]/2)):#Loop through each column range/2
                ...
                maxIndex = getIndex(df, col, maxClass, options, True)
                minIndex = getIndex(df, col, minClass, options, False)
...
for i in range (numAttempts): # Start the loop for each attempt
    ...
    df = normaliseNames(df, 'Name')
    for col in options.keys():
        if str(options[col]) in ['-1', '1']:
            df = normaliseNames(df, col)#Normalise names for a column of a DF
